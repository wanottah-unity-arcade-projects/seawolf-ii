...
public class Player1TorpedoController2 : MonoBehaviour
{
    ...
    {
        Vector3 direction = (Player1Controller.player1.torpedoTargetPosition[TORPEDO_2] - transform.position).normalized;

        transform.position += direction * torpedoSpeed * Time.deltaTime;

        if (Vector3.Distance(Player1Controller.player1.torpedoTargetPosition[TORPEDO_2], transform.position) <= 0.1f)
        {
            gameObject.SetActive(false);

            Player1Controller.player1.torpedoTarget[TORPEDO_2].gameObject.SetActive(false);
        }
    }
